...
function UPnPGUI() {
    ...
    this.setPlayerButtons = function(state) {
        ...
        switch(state) {
            ...
            case 'stop':
                ...
                $('#track-cover').hide().empty();
                $('#track-title').text(upnp.gui.i18n('- keine Wiedergabe aktiv - '));
        ...
        upnp.backend.call(this.dstDevice.getUid(), 'getPositionInfo', null, function(res) {
            ...
            if(undefined != data.title) {
                ...
                if(albumArtURI.indexOf('file.tbn') > -1) {
                    ...
                    // item has cover
                    if(data.albumArtURI.indexOf('image%3A%2F%2F') > -1) {
                        albumArtURI = decodeURI(data.albumArtURI).split('image%3A%2F%2F');
                        albumArtURI = decodeURIComponent(albumArtURI[1]);
                    }
                }

                $('#track-cover').attr('src',albumArtURI);
                $('#track-cover').show();
                $('#track-title').text(data.artist+' - '+data.title);
    ...
    this.loadDevices = function() {
        ...
        upnp.backend.call(null, 'getDevices', null, function(res) {
            ...
            $(dropdown_src).append($('<option value="">Quelle auswählen</option>'));
            $(dropdown_dst).append($('<option value="">Gerät auswählen</option>'));
            ...
            $('#ds-src').append(dropdown_src);
            $('#ds-dst').append(dropdown_dst);
    ...
    this.deviceChanged = function(dropdown) {
        dropdown = $(dropdown).find("option[selected='selected']");
    ...
    this.loadRenderer = function(device) {
        ...
        var name = '- keine Wiedergabe aktiv -';

        var controls = $('<div id="controls"></div>');
        var title = $('<div class="title"><img src="res/images/icons/ajax-loader-player.gif" id="player-loading"><img id="track-cover" src="" style="display:none;"><span id="track-title">' + name + '</span></div>');
    ...
    this.drawFiletable = function() {
        ...
        html+= '      <span class="add-on" style="height: 24px; border-radius: 0px;"><i class="icon-search"></i></span>';
        html+= '      <input class="span6" id="searchbar" type="text" style="height: 24px; border-radius: 0px;">';
        //html+= '      <img id="search-enter" src="res/images/icons/keyboard-enter.png" style="display: none;" />';
        html+= '    </div>';
        html+= '  </div>';
        html+= '</div>';
        ...
        if(cnt > 0) {
            ...
            for(var id in files) {
                ...
                var html = '<a id="item-' + file.getId() + '" href="javascript:upnp.filemanager.' + (file.getType() == 'container' ? 'open' : 'details') + '(\'' + file.getId() + '\');"><div class="filerow">';
                html += '   <div class="mime">';
                if (file.getClass() == "object.container.album.musicAlbum" && file.getAlbumArtURI() != undefined)
                    html += '       <img title="' + file.getClass() + '" alt="' + file.getClass() + '" src="' + file.getAlbumArtURI() + '" />';
                else
                    html += '       <img title="' + file.getClass() + '" alt="' + file.getClass() + '" src="res/images/icons/mime/' + file.getClass() + '.png" />';
                html += '   </div>';
                html += '       <div class="text">' + file.getTitle() + '</div>';
                html += '   <div class="right"></a>';
