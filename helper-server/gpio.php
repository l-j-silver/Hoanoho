...
if (isset($_GET['cmd'])) {
    if ($_GET['cmd'] == "set" && isset($_GET['pin']) && isset($_GET['value'])) {
        ...
        if (!$portPresent) {
            // initialize port
            shell_exec('sudo '.getcwd().'/../helper-server/gpio.sh enable out '.trim($_GET['pin']));
        }
        ...
        shell_exec('sudo '.getcwd().'/../helper-server/gpio.sh set out '.trim($_GET['pin']).' '.$value);
    ...
    } elseif ($_GET['cmd'] == "get" && isset($_GET['pin'])) {
